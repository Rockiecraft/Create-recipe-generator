...
function captureLayoutSnapshot(recipeId, engineKey) {
  ...
  const isPluginEngine = !!(window.PLUGIN_DISPATCH && window.PLUGIN_DISPATCH[engineKey]);

  if (isPluginEngine) {
    delete recipe.domSnapshot[engineKey];
  } else if (['mixing', 'compacting'].includes(eng)) {
  ...
  // Plugin engines never get a DOM snapshot (see above) — everything else
  // (name/platform/conditions sync below, enginesData save) still applies.
  if (!isPluginEngine) {
...
function applyLayoutSnapshot(recipeId, engineKey) {
  ...
  const isPluginEngine = !!(window.PLUGIN_DISPATCH && window.PLUGIN_DISPATCH[engineKey]);
  if (isPluginEngine) return false;
